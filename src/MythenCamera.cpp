...
void Camera::CameraThread::execCmd(int cmd)
{
    ...
    switch (cmd)
    {
        ...
        case Refresh:
            ...
            break;
        case SetSettings:
            cout << "Camera::CameraThread::execCmd() case SetSettings" << endl;
            if (status != Ready && !m_cam->ACBusyFlag && !m_cam->RBusyFlag)
                throw LIMA_HW_EXC(InvalidValue,  "Not Ready to SetSettings");
            execSetSettings();
            break;
    }
}
...
{
    cout << "Camera::CameraThread::execStartAcq() entering...." << endl;
    int acq_frame_nb;
    ...
    while(nb_frames)
    {
        ...
        while(intStat != 0 and m_force_stop != true);
        ...
        while (myfile.good())
        {
            ...
            if (idx>=0 && idx<MAXSTR) m_cam->m_data[idx]=tmpdat; // Copy data to the array
        }
        myfile.close();
        ...
        {
            sprintf(cmd, "rm -f %s", file_to_open);
            system(cmd);
    ...
    cout << "Camera::CameraThread::execStartAcq() leaving...." << endl;
}

void Camera::CameraThread::execStartStartAcq()
{
    cout << "Camera::CameraThread::execStartStartAcq() entering...." << endl;

    if( !m_cam->cam_sim_mode )
    {
        char *myArgs[1] = {""};
        multiSlsDetectorCommand *cmd=new multiSlsDetectorCommand(m_cam->m_detector);
        cmd->executeLine(0, (char **)myArgs, READOUT_ACTION);
    ...
    cout << "Camera::CameraThread::execStartStartAcq() leaving...." << endl;
}


void Camera::CameraThread::execRefresh()
{
    cout << "Camera::CameraThread::execRefresh() entering..." << endl;
    float FVar;
    string answer;
    ...
    if(!m_cam->cam_sim_mode)
    {
        multiSlsDetector* myDetector = m_cam->m_detector;

        answer=m_cam->mythen_get("flatfield"); // TODO: crash... write PSI people
        ...
        for (int i=0; i<m_cam->m_n_modules; ++i) 
        {
            ...
            answer=m_cam->mythen_get(tmpstr1);
            if (i==0)
        ...
        answer=m_cam->mythen_get("moduleversion");
        m_cam->m_module_firmware_version = atoi(answer.c_str());
        answer=m_cam->mythen_get("detectornumber");
        m_cam->m_detector_serial_number = atoi(answer.c_str());
        answer=m_cam->mythen_get("detectorversion");
        m_cam->m_detector_firmware_version = atoi(answer.c_str());
        answer=m_cam->mythen_get("softwareversion");
        m_cam->m_detector_software_version = atoi(answer.c_str());
        answer=m_cam->mythen_get("thisversion");
        m_cam->m_mythen_software_version = atoi(answer.c_str());
        answer=m_cam->mythen_get("index");
        m_cam->m_out_fileindex = atoi(answer.c_str());

        answer=m_cam->mythen_get("settings");
        m_cam->m_settings = answer;

        answer=m_cam->mythen_get("threshold");
        m_cam->m_threshold = atoi(answer.c_str());
        cout << "Camera::CameraThread::execRefresh() threshold = " << m_cam->m_threshold << endl;
        answer=m_cam->mythen_get("exptime");
        m_cam->m_exposure = atof(answer.c_str());
        answer=m_cam->mythen_get("period");
        m_cam->m_frame_period = atof(answer.c_str());
        answer=m_cam->mythen_get("delay");
        m_cam->m_delay_after_trigger = atof(answer.c_str());
        answer=m_cam->mythen_get("gates");
        m_cam->m_gates_per_frame = atoi(answer.c_str());
        answer=m_cam->mythen_get("frames");
        m_cam->m_frames_per_cycle = atoi(answer.c_str());
        answer=m_cam->mythen_get("cycles");
        m_cam->m_n_cycles = atoi(answer.c_str());
        answer=m_cam->mythen_get("probes");
        m_cam->m_n_probes = atoi(answer.c_str());
        answer=m_cam->mythen_get("dr");
        m_cam->m_dynamic_range = atoi(answer.c_str());
        answer=m_cam->mythen_get("online");
        m_cam->m_online = atoi(answer.c_str());

        answer=m_cam->mythen_get("flags");
        m_cam->m_flags = answer;

        //      *m_TrimMode_read: cannot be read
        answer=m_cam->mythen_get("trimdir");
        m_cam->m_trim_dir = answer;

        answer=m_cam->mythen_get("outdir");
        m_cam->m_out_dir = answer;

        answer=m_cam->mythen_get("fname");
    ...
    cout << "Camera::CameraThread::execRefresh() leaving..." << endl;
}

void Camera::CameraThread::execSetSettings()
{
    setStatus(Config);
    const char *myArgs[2] = {"settings", m_cam->m_settings.c_str()};
    multiSlsDetectorCommand *cmd=new multiSlsDetectorCommand(m_cam->m_detector);
    cmd->executeLine(2, (char **)myArgs, PUT_ACTION);
    setStatus(Ready);
    return;
}
...
Camera::Status Camera::getStatus()
{
    ...
    switch (thread_status)
    {
        case CameraThread::Ready:
        {
            ...
            if (refresh_status == CameraThread::Readout)
                status = Camera::Readout;
            else
                status =  Camera::Ready;
            break;
        }
        ...
        case CameraThread::Latency:
            status = Camera::Latency;
            break;
        case CameraThread::Config:
            status = Camera::Config;
            break;
        default:
...
//     when executing a command when one command is already runnning, a broken pipe kills the server...
//     char *myArgs[2] = {"status", "stop"};
//     multiSlsDetectorCommand *cmd=new multiSlsDetectorCommand(m_detector);
//     cout << "Camera::stopAcq() before executing 'status stop'" << endl;
//     cmd->executeLine(2, (char **) myArgs, PUT_ACTION);
...
void Camera::new_mythen_detector()
{
    ...
    if (!m_detector || first || DetId!=m_detector_id) 
    {
        ...
        try
        {
            m_detector = new multiSlsDetector(m_detector_id);
...
string Camera::MythenExecuteLine(int Action) 
{
    ...
    try 
    {
        // Send the command
        multiSlsDetectorCommand *cmd=new multiSlsDetectorCommand(m_detector);
        cmd->executeLine(0, (char **)myArgs, Action);
    } 
    catch (exception& e)
    {
...
//    if (getBusyFlag()) return "";
    ...
    //multiSlsDetector *multidet=new multiSlsDetector(0);
    //multiSlsDetectorCommand *cmd=new multiSlsDetectorCommand(multidet);
    multiSlsDetectorCommand *cmd=new multiSlsDetectorCommand(m_detector);
    answer = cmd->executeLine(1, (char **)myArgs, Action);
    }
    catch (exception& e) 
    {
        cout << "Camera::MythenExecuteLine() exception: " << e.what() << endl;
...
string Camera::MythenExecuteLine(string line1, string line2, int Action)
{
    ...
    try 
    {
      // Send the command
        multiSlsDetectorCommand *cmd=new multiSlsDetectorCommand(m_detector);
        cmd->executeLine(2, (char **)myArgs, Action);
    } catch (exception& e)
    {
        cout << "Camera::MythenExecuteLine() exception: " << e.what() << endl;
    }
...
void Camera::mythen_set(string var)
{
    ...
    else if (var=="angconv") 
    {
        ...
        strcpy(myArgs[1], m_ang_conv_filename.c_str());
	        ...
	        var=="probes" || var=="dr" || var=="trimdir" || var=="caldir" || var=="outdir" ||
	        var=="fname" || var=="index" || var=="threaded" || var=="hostname" || var=="port" ||
	        var=="binsize" || var=="online" || var=="settings")
    {
        myNArg=2;
        myArgs[0] = new char[16];
        strcpy(myArgs[0],var.c_str());
        ...
        if (var=="globaloff")
            ...
            strcpy(myArgs[1], m_settings.c_str());
    ...
    if (!cam_sim_mode && m_detector)
    {
        try
        {
            // 2. Send the command
            multiSlsDetectorCommand *cmd=new multiSlsDetectorCommand(m_detector);
            answer = cmd->executeLine(myNArg, (char **)myArgs, PUT_ACTION);
            if (var=="threshold") 
                m_threshold = atoi(answer.c_str()); // the actual set value
        }
        catch (exception& e)
        {
...
string Camera::mythen_get(char *parameter) 
{
    char *myArgs[1];
    ...
    try
    {
        ...
        multiSlsDetectorCommand *cmd=new multiSlsDetectorCommand(m_detector);
        cout << "Camera::mythen_get()" << myArgs[0] << endl;
        answer = cmd->executeLine(1, (char **)myArgs, GET_ACTION);
        cout << "Camera::mythen_get()" << myArgs[0] << answer;
    }
    catch (exception& e)
    {
        answer="";
...
void Camera::setSettings(std::string settings)
{
    DEB_MEMBER_FUNCT();
    m_settings = settings;
    //done with an external thread, since it takes some time to change settings... (more than 3 sek)
    m_thread.sendCmd(CameraThread::SetSettings);
}

std::string Camera::getSettings()
{
    DEB_MEMBER_FUNCT();
    std::string settings = getStringParameter("settings");
    return settings;
}
...
void Camera::setIntParameter(std::string parameter, int value)
{
    ...
    else if (parameter == "detectornumber" || parameter == "detectorversion" 
            || parameter == "softwareversion" || parameter == "thisversion" )
...
{
    std::string parameter_list = "cycles,detectornumber,detectorversion,dr,frames,gates,index,maxmod, \
                                moduleversion,nmod,online,probes,softwareversion,thisversion,threaded";
...
{
    std::string parameter_list ="angconv,badchannels,caldir,fname,outdir,trimdir";
    return parameter_list;
}